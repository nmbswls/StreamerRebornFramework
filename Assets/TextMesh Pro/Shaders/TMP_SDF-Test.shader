...
Shader "TextMeshPro/Mobile/Test" {
...
SubShader {
	...
	Pass {
		...
		struct pixel_t {
			...
			#endif
			float4	srcPos	: TEXCOORD5;			// Scale(x), Bias(y)
		};
		...
		pixel_t VertShader(vertex_t input)
		{
			...
			output.srcPos = ComputeScreenPos(output.vertex);
		...
		fixed4 PixShader(pixel_t input) : SV_Target
		{
			...
			float2 noiseUV = float2(input.srcPos.xy * _NoiseTex_ST + _NoiseTex_ST.zw);
			...
			fixed edgeLerp = step(factor, c.a) * saturate((_DissolveWidth - factor) * 16 / _DissolveSoftness);
			half4 temp = fixed4(_DissolveColor.rgb, edgeLerp);
			c.rgb += temp.rgb * temp.a;
			c.a *= saturate((factor) * 32 / _DissolveSoftness);
